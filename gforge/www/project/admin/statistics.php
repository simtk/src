...
<?php 
    ...
    //echo $HTML->boxTop(_('<h3>Statistics</h3>'));
...
?>
	...
	<?php foreach ($resultsDownloads as $results) { ?>
	  <?php $userprofile = "<a href='/users/".$results['user_name']."'>".$results['firstname'] . " " . $results['lastname'] . "</a>"; ?>
	  <tr><td><?php echo $results['package_name']; ?></td><td><?php echo $results['release_name']; ?></td><td><?php echo $results['filename']; ?></td><td><?php echo $results['firstname'] != 'Nobody' ? $userprofile : "Not logged."; ?></td><td><?php echo $results['university_name']; ?></td><td><?php echo $results['lab_name']; ?></td><td><?php echo $results['expected_use']; ?></td><td><?php echo $results['date']; ?></td><td><?php echo $results['agreed_to_license'] ? "Yes" : "No"; ?></td></tr>
	<? } ?>
	</table>
	</div>
	<br />
	...
	if ($limit == 100 && $limit <= $numrows) {
	  echo "<a href='statistics.php?group_id=" . $group->getID() . "&offset=" . $offset . "'>More....</a>";
	}
