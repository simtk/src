...
//	'S_DISPLAY_POST_INFO'	=> ($forum_data['forum_type'] == FORUM_POST && ($auth->acl_get('f_post', $forum_id) || $user->data['user_id'] == ANONYMOUS)) ? true : false,
	'S_DISPLAY_POST_INFO'	=> ($forum_data['forum_type'] == FORUM_POST && (true || $user->data['user_id'] == ANONYMOUS)) ? true : false,
...
//	'U_POST_NEW_TOPIC'	=> ($auth->acl_get('f_post', $forum_id) || $user->data['user_id'] == ANONYMOUS) ? append_sid("{$phpbb_root_path}posting.$phpEx", 'mode=post&amp;f=' . $forum_id) : '',
	'U_POST_NEW_TOPIC'	=> (true || $user->data['user_id'] == ANONYMOUS) ? append_sid("{$phpbb_root_path}posting.$phpEx", 'mode=post&amp;f=' . $forum_id) : '',
...
//			'FIRST_POST_TIME'			=> $user->format_date($row['topic_time']),
			'FIRST_POST_TIME'			=> ($user->data['user_id'] == 1 || $user->data['user_id'] == 2) ? ($user->format_date($row['topic_time'] - 3600*8)) : ($topicTime = $user->format_date($row['topic_time'])),
			'LAST_POST_SUBJECT'			=> censor_text($row['topic_last_post_subject']),
//			'LAST_POST_TIME'			=> $user->format_date($row['topic_last_post_time']),
			'LAST_POST_TIME'			=> ($user->data['user_id'] == 1 || $user->data['user_id'] == 2) ? ($user->format_date($row['topic_last_post_time'] - 3600*8)) : ($topicTime = $user->format_date($row['topic_last_post_time'])),
//			'LAST_VIEW_TIME'			=> $user->format_date($row['topic_last_view_time']),
			'LAST_VIEW_TIME'			=> ($user->data['user_id'] == 1 || $user->data['user_id'] == 2) ? ($user->format_date($row['topic_last_view_time'] - 3600*8)) : ($topicTime = $user->format_date($row['topic_last_view_time'])),
