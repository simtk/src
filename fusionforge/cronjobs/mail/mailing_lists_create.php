...
for ($i=0; $i<$rows; $i++) {
	...
	if ($status == MAIL__MAILING_LIST_IS_REQUESTED) {	// New list?
		...
		if ($failed) {
			...
			$listConfig .= "mlist.moderator.append('" . $email . "')\n" ;
			if (!$public) {
		...
		$change_pw_cmd = escapeshellcmd($path_to_mailman .
			'/bin/change_pw -q -l ' . $listname .
			' -p ' . $listpassword);
		$err .= "Resetting password of " . $listname."\n";
		exec($change_pw_cmd, $returnpasswd, $failed);
		if ($failed) {
			$err .= 'Failed to reset password of ' . $listname."\n";
		}
		else {
			// Reset password to the password which is stored in the databases and returned.
			$arrayReturnPasswd = explode(' ', $returnpasswd[0]);
			$retpasswd = trim(end($arrayReturnPasswd));
			db_query_params('UPDATE mail_group_list set (status, password) = ($1, $2)  ' .
				'WHERE status=$3 and group_list_id=$4',
				array(MAIL__MAILING_LIST_IS_CONFIGURED,
					$retpasswd,
