...
/**
 * group_get_public_active_projects_asc() - Get a list of rows for public active projects (initially in trove/full_list)
 *
...
class Group extends FFError {
	...
	function update(&$user, $group_name, $homepage, $short_description, $use_mail, $use_survey, $use_forum,
		...
		$simtk_summary, $simtk_download_description, $logo_tmpfile, $logo_type) {
	...
	function approve(&$user) {
		...
		if ($template) {
			...
			foreach ($template->getRoles() as $oldrole) {
				...
				$sections = array('project_read', 'project_admin', 'scm', 'docman', 'tracker_admin', 'new_tracker', 'forum_admin', 'new_forum', 'pm_admin', 'new_pm', 'frs_admin', 'new_frs', 'datashare');
			...
			db_query_params ('UPDATE groups SET use_mail=0, use_survey=0, use_forum=0, use_pm=0, use_pm_depend_box=0, use_news=0, use_ftp=0, use_stats=0 WHERE group_id=$1',
				array($this->getID()));
		}
