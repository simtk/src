	...
	function user_act_daily($day) {
		...
		return db_query_params ('INSERT INTO rep_user_act_daily
			...
			FULL OUTER JOIN
				...
				GROUP BY user_id) tclosed USING (user_id,day)) foo6',
					array($day,
						$end_day,
						date ('Ym'),
						1,
	...
	function group_act_daily($day) {
		...
		FROM
			...
			FULL OUTER JOIN
				...
				AND a.group_artifact_id=agl.group_artifact_id
				GROUP BY group_id) aclosed USING (group_id,day)) foo1
			...
			FULL OUTER JOIN
				...
				GROUP BY group_id) forum USING (group_id,day)) foo2

			FULL OUTER JOIN
				(SELECT group_id, $1::int AS day, count(*) AS docs
				FROM doc_data
				WHERE createdate BETWEEN $1 AND $2
				GROUP BY group_id) docs USING (group_id,day)) foo3
			...
			FULL OUTER JOIN
				...
				GROUP BY fp.group_id) docs_downloads USING (group_id,day)) foo4

			FULL OUTER JOIN
				(SELECT group_id, $1::int AS day, sum(commits) AS cvs_commits
				FROM stats_cvs_group
				WHERE month=$3 AND day=$4
				GROUP BY group_id) cvs USING (group_id,day)) foo5
			...
			FULL OUTER JOIN
				...
				GROUP BY group_id) topen USING (group_id,day)) foo6
			...
			FULL OUTER JOIN
				...
				AND pt.group_project_id=pgl.group_project_id
				GROUP BY group_id) tclosed USING (group_id,day)) foo7
			...
			FULL OUTER JOIN
				...
				AND fdf.simtk_host_name like \'%.stanford.edu\'
                                GROUP BY fp.group_id) docs_stan USING (group_id,day)) foo44',
					array($day,
						$end_day,
						date('Ym', $day),
						date('d',$day),
