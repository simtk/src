	...
	*/
	...
	function create($name, $file_location, $type_id, $processor_id, $release_time=false, 
		...
		if ($launchGitHubCronJob === true) {
			...
			system("/usr/bin/php /usr/share/gforge/cronjobs/db/getGitHubArchive.php " .
				$this->getID() . 
				" >> /dev/null 2>&1 &");
			*/
			// Retrieve GitHub archive file.
			$status = $this->getGitHubArchive($this->getID());
			if ($status === false) {
				// Cannot retrieve archive file.
				$this->setError("Cannot retrieve GitHub archive file");
			}
			else {
				// File released.
				if ($emailChange == 1) {
	 ...
	 * @return	boolean	success.
	 */
	function delete($isDeleteRelease=false, $emailChange=1) {
		if (!forge_check_perm ('frs', $this->FRSRelease->FRSPackage->Group->getID(), 'write')) {
			$this->setPermissionDeniedError();
			return false;
		}
	...
	function copyToRelease($newRelease, $newFile) {
		...
		if ($newFile && !$this->isURL()) {
			...
			$toFileDir = forge_get_config('upload_dir') . '/' . 
				...
				$newRelease->FRSPackage->getFileName() . '/' .
				$newRelease->getFileName() . '/';
	...
	function getGitHubArchive($fileId) {
