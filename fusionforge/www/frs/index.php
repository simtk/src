...
if (session_loggedin()) {
	...
	if ($package_id != 0) {
		if ($start === 1) {
			$tmpPackName = $package_id;
			$tmpPack = frspackage_get_object($package_id);
			if ($tmpPack !== false) {
				$tmpPackName = $tmpPack->getName();
			}
			$warning_msg = "You have started following package \"" . $tmpPackName . "\".";
		}
		else if ($stop === 1) {
			$tmpPackName = $package_id;
			$tmpPack = frspackage_get_object($package_id);
			...
			$warning_msg = "You have stopped following package \"" . $tmpPackName . "\".";
		}
	}
}
else {
