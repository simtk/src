...
// Study
$result_pending_study = db_query_params("SELECT group_name, title, " .
	"doi_identifier, doi_requester, study_id " .
	"FROM plugin_datashare pd " .
	"JOIN groups g " .
	"ON g.group_id = pd.group_id " .
	"WHERE pd.doi = 1 " .
	"AND pd.doi_identifier is null",
	array());
$result_assigned_study = db_query_params("SELECT group_name, title, " .
	"doi_identifier " .
	"FROM plugin_datashare pd " .
	"JOIN groups g " .
	"ON g.group_id = pd.group_id " .
	"WHERE pd.doi = 1 ".
	"AND pd.doi_identifier <> '' ",
	array());
...
if (getStringFromRequest('submit')) {
	...
	else if (!empty($study_user_id) && !empty($study_id)) {
		// Study.
	...
	if (!$result || db_affected_rows($result) < 1) {
		...
		if ($file_user_id || $package_user_id || $study_user_id) {
			...
			else if ($package_user_id) {
				$message .= "Package: ";
			}
			else if ($study_user_id) {
				$message .= "Study: ";
			}
...
while ($row = db_fetch_array($result_pending_study)) {
	echo "<form action='downloads-doi.php' method='post'>" .
		...
		"<input type='hidden' name='study_user_id' value='" . $row['doi_requester'] . "'>" .
		"<input type='hidden' name='title' value='" . $row['title'] . "'>" .
		"<input type='hidden' name='group_name' value='" . $row['group_name'] . "'>" .
		"<tr>" .
		"<td>". $row['group_name'] . "</td>";
	echo "<td>";
	if ($row['doi_requester']) {
		$user = user_get_object($row['doi_requester']);
		if ($user) {
			$real_name = $user->getRealName();
			echo $real_name;
		}
...
// Study.
while ($row = db_fetch_array($result_assigned_study)) {
	echo "<tr>" .
		"<td>" . $row['group_name'] . "</td>" .
		"<td>" . $row['title'] . "</td>" .
		"<td>" . $row['doi_identifier'] . "</td>" .
