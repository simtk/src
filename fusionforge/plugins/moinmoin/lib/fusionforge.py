...
class FusionForgeSessionAuth(BaseAuth):
    ...
    def __init__(self, cookies=['session_ser'], autocreate=True):
        ...
        cur.execute("""SELECT distinct(u.user_name)
                       FROM users u 
                       JOIN pfo_user_role pur ON u.user_id=pur.user_id 
                       JOIN pfo_role pr ON pur.role_id=pr.role_id
                       JOIN pfo_role_setting prs ON prs.role_id=pr.role_id
                       WHERE prs.section_name='forge_admin' AND prs.perm_val = 1""")
        self.admins = []
        if cur.rowcount > 0:
        ...
        cur.execute("""SELECT g.unix_group_name FROM groups g
                       JOIN group_plugin gp ON g.group_id=gp.group_id
                       JOIN plugins p ON p.plugin_id=gp.plugin_id
                       WHERE p.plugin_name = 'moinmoin'""")
        self.projects = []
        if cur.rowcount:
    ...
    def get_moinmoin_acl_string(self, project_name):
        ...
        query = """SELECT prs.perm_val FROM pfo_role_setting prs
                       JOIN groups ON groups.group_id=prs.ref_id
                       WHERE prs.role_id = 1
                         AND prs.section_name = '%s'
                         AND groups.unix_group_name = '%s' """ \
                    % (perm_name, project_name)
    ...
    def get_permission_entries (self, project_name, perm, user_name = None):
        ...
        query = """SELECT DISTINCT user_name FROM
                       ...
                       WHERE %s
                         AND prs.section_name = '%s'
                         AND groups.unix_group_name = '%s'
                         AND prs.perm_val = %d) AS q1
                         UNION
                       (SELECT DISTINCT u.user_name FROM 
                       (SELECT user_id, user_name FROM users) u
                       JOIN pfo_user_role pur ON u.user_id=pur.user_id,
                       pfo_role pr, pfo_role_setting prs, 
                       (SELECT group_id, unix_group_name FROM groups) g
                       WHERE %s
                         AND prs.role_id = 2
                         AND prs.section_name = '%s'
                         AND g.unix_group_name = '%s'
                         AND prs.perm_val = %d
                         AND prs.ref_id = g.group_id)""" \
                    % (ucond, perm_name, project_name, perm_map[perm], ucond, perm_name, project_name, perm_map[perm])
