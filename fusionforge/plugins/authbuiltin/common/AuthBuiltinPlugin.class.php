...
class AuthBuiltinPlugin extends ForgeAuthPlugin {
	...
	function displayAuthForm(&$params) {
		...
		$result .= html_e('br').html_e('input', array('type' => 'text', 'class' => 'loginname', 'name' => 'form_loginname', 'value' => htmlspecialchars(stripslashes($loginname)), 'required' => 'required'));
		...
		$result .= html_ac(html_ap() -1);
		$result .= html_e('p', array(), html_e('input', array('type' => 'submit', 'class' => 'btn-cta', 'name' => 'login', 'value' => _('Login'))), false);
		$result .= $HTML->closeForm();
		$result .= html_e('p', array(), util_make_link('/account/lostpw.php', 'Reset your password'));
		// hide "new account" item if restricted to admin
		if (!forge_get_config ('user_registration_restricted')) {
			$result .= html_e('p', array(), util_make_link('/account/register.php', 'Create new account'));
		}
