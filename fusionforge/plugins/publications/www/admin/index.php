	...
	if (isset($action)) {
		if ($action == "delete") {
			...
			if ($group->isPublicationProject()) {
				...
				// Check if primary publication will exist after deletion.
				$hasPrimary = false;
				$result = $pub->getPublications();
				if ($result) {
					foreach ($result as $result_list) {
						if ($result_list->is_primary) {
							// Found primary publication.
							if ($result_list->pub_id == $pub_id) {
								// Yet, this publication will be deleted.
							}
							else {
								// Will have primary publication remaining after deletion.
								$hasPrimary = true;
							}
						}
					}
		...
		elseif ($action == "unsetprimary") {
			// Unset primary is available only to non-publication project.
